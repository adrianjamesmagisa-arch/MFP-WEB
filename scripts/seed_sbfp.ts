  ...
  for (const [sheetName, centerCode] of Object.entries(CENTER_SHEETS)) {
    ...
    for (let i = headerRow + 1; i < data.length; i++) {
      ...
      const record: any = {
        ...
        beneficiaries_pm: Math.round(Number(row[10])) || 0,
        contract_amount: Math.round(Number(row[11])) || 0,
        delivery_start: excelDateToISO(row[12]) || null,
        delivery_end: excelDateToISO(row[13]),
        packs_to_deliver: Math.round(Number(row[14])) || 0,
        ...
        delivery_schedule: typeof row[12] === 'string' ? row[12] : '',
      };
    ...
    if (headerRow !== -1) {
      ...
      for (let i = headerRow + 1; i < data.length; i++) {
        ...
        allRecords.push({
          ...
          beneficiaries_pm: Math.round(Number(row[9])) || 0,
          contract_amount: Math.round(Number(row[10])) || 0,
          packs_to_deliver: Math.round(Number(row[13])) || 0,
          packs_delivered: snapshots.length > 0 ? Math.round(snapshots[snapshots.length - 1].packs) : 0,
