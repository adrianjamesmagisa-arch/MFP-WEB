...
const sC:  React.CSSProperties = { position: 'sticky', left: 281, zIndex: 6, width: 160, minWidth: 160, maxWidth: 160, boxShadow: '3px 0 6px -2px rgba(0,0,0,0.18)' }`
  ...
  // Now replace the row cells
  code = code.replace(/<td style=\{\{ \.\.\.iCell, \.\.\.sA, zIndex: 5 \}\}>\s*<select\s+className="nodrag"\s+style=\{cSelect\}\s+value=\{row\.year\}[\s\S]*?<\/select>\s*<\/td>\s*<td style=\{\{ \.\.\.iCell, \.\.\.sB, zIndex: 5 \}\}>/g,
    `<td style={{ ...iCell, ...sA, zIndex: 5 }}>`
  );

  code = code.replace(/<td style=\{\{ \.\.\.iCell, \.\.\.sC, zIndex: 5 \}\}>/g,
    `<td style={{ ...iCell, ...sB, zIndex: 5 }}>`
  );

  code = code.replace(/<td style=\{\{ \.\.\.iCell, \.\.\.sD, zIndex: 5, borderRight: dividerBorder \}\}>\s*<select\s+className="nodrag"\s+style=\{cSelect\}\s+value=\{row\.center\}[\s\S]*?<\/select>\s*<\/td>\s*<td style=\{iCell\}>\s*<select\s+className="nodrag"\s+style=\{cSelect\}\s+value=\{row\.province\}/g,
    `<td style={{ ...iCell, ...sC, zIndex: 5, borderRight: dividerBorder }}>
