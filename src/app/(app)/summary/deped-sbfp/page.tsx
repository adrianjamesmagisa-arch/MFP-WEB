...
export default async function SummaryDepEdPage() {
  ...
  let query = supabase
    .from('mfp_data')
    .select('year, region, province, division, supplier_id, beneficiaries, milk_packs, raw_milk_liters, milk_cost, service_fee, total_funds_transferred, feeding_days, milk_type, center')
    .eq('funded_by', 'DepEd')
  ...
  function forYear(yr: number) {
    ...
    return {
      ...
      sdos:             new Set(rows.map(r => r.division)).size,
      ...
      serviceFee:       rows.reduce((s, r) => s + (r.service_fee || 0), 0),
  ...
  const rowDefs: { label: string; key: keyof typeof params[0]; currency?: boolean; highlight?: boolean }[] = [
    ...
    { label: 'No. of School Division Offices',     key: 'sdos' },
    ...
    { label: 'Amount of Service Fee',              key: 'serviceFee',       currency: true },
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-title">Summary: DepEd-SBFP</h1>
          <p className="page-subtitle">
            {isEncoder && centerFilter
              ? <>📍 <strong>{centerFilter}</strong> Center — DepEd School-Based Feeding Program</>
              : 'Department of Education — School-Based Feeding Program (All Centers)'}
          </p>
        </div>
        {isEncoder && centerFilter && (
          <div style={{ background: '#1d4ed8', color: 'white', borderRadius: 10, padding: '0.6rem 1.25rem', fontSize: '0.82rem', fontWeight: 700 }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="summary-table">
            ...
            <tbody>
              ...
              <tr>
                <td style={{ fontWeight: 800, background: 'var(--gold)', color: 'var(--navy)', borderLeft: 'none' }}>TOTAL</td>
                {params.map(p => (
                  <td key={p.year} style={{ fontWeight: 800, background: '#fef3c7', color: 'var(--navy)' }}>
      ...
      {params.length === 0 && (
        <div className="card" style={{ padding: '3rem', textAlign: 'center', color: 'var(--gray-400)', marginTop: '1rem' }}>
          No DepEd records found{centerFilter ? ` for ${centerFilter}` : ''} yet.
        </div>
      )}
    </div>
  )
