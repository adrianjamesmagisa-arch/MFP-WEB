...
export default async function SummaryDepEdPage(props: {
  ...
  let query = supabase
    .from('mfp_data')
    .select('beneficiaries, milk_packs, milk_cost, total_funds_transferred, funded_by, year, center, region, province, division, municipality, elementary_school, feeding_days, batch, date_started')
    .eq('funded_by', 'DepEd')
    .range(0, 49999)
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">School-Based Feeding Program · All Centers Combined</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        {[
          { label: 'Total Beneficiaries', value: formatNumber(totalBene),   color: '#1d4ed8' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div className="card" style={{ padding: '1.5rem' }}>
          <h3 style={{ fontWeight: 700, color: '#1d4ed8', marginBottom: '1.25rem', fontSize: '1rem' }}>
              ...
              return (
                <div key={r.region}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: '0.2rem' }}>
                    ...
                    <span style={{ color: '#1d4ed8', fontWeight: 700 }}>{formatNumber(r.bene)}</span>
                  </div>
              ...
              <thead>
                <tr style={{ background: '#dbeafe' }}>
                  {['Center', 'Beneficiaries', 'Milk Packs', 'Gross Income', 'Records'].map(h => (
                    <th key={h} style={{ padding: '0.5rem 0.75rem', textAlign: h === 'Center' ? 'left' : 'right', color: '#1d4ed8', fontWeight: 700 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
