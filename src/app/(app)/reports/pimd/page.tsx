...
export default function PIMDReportPage() {
  ...
  async function captureArtboard(): Promise<HTMLCanvasElement> {
    const el = document.getElementById('pimd-factsheet') as HTMLElement
    if (!el) throw new Error('Artboard not found')
    ...
    return h2c(el, {
      ...
      onclone: (doc: Document) => {
        const cl = doc.getElementById('pimd-factsheet')
        if (!cl) return

        // ── Root artboard: remove only the screen-scale transform ──
        // Do NOT touch child elements — their transforms are for centering.
        cl.style.transform = 'none'
        cl.style.transformOrigin = 'top left'
        cl.style.width = `${ARTBOARD_WIDTH}px`
        cl.style.height = `${ARTBOARD_HEIGHT}px`
        cl.style.position = 'relative'
        cl.style.left = '0'
        cl.style.top = '0'
        cl.style.overflow = 'hidden'   // clip only at the outer 1414×2000 boundary

        // ── Ensure no descendant value containers clip glyphs ──────
        // html2canvas clips any pixel outside an overflow:hidden box,
        // including the top of ascenders and the bottom of descenders.
        // We must NOT bulk-modify transforms or positions — only overflow.
        cl.querySelectorAll<HTMLElement>(
          // Target the FittedText wrapper divs and any explicit value wrappers.
          // They are identified by the combination of white-space:nowrap and display:block/inline.
          '[style*="overflow: hidden"], [style*="overflow:hidden"]'
        ).forEach(node => {
          // Only relax overflow on leaf value nodes, not on structural containers
          // that need overflow:hidden for border-radius clipping (e.g. .pimd-dswd-centers).
          // Safe heuristic: if the node has no class containing 'pimd-', it is a value wrapper.
          const hasStructuralClass = node.className &&
            typeof node.className === 'string' &&
            node.className.includes('pimd-')
          if (!hasStructuralClass) {
            node.style.overflow = 'visible'
          }
        })

        // Hide the reference overlay if active
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Get actual dimensions from blob
      const img = new Image()
      img.src = dataUrl
  ...
  const handlePrintFromModal = () => {
    ...
    // Use @media print to isolate only the preview image on one A4 sheet
    ...
    // Create hidden print frame if not present
        ...
        <div className="pimd-modal-backdrop no-print" onClick={closePreviewModal}>
          <div className="pimd-modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="pimd-modal-footer">
              ...
              <button onClick={handlePrintFromModal}
