...
export async function GET() {
  ...
  const ready = !procErr && !dropErr && !mfpErr && !expandErr
  ...
  if (!ready) {
    ...
    if (procErr && existsSync(BASE_SQL)) chunks.push(readFileSync(BASE_SQL, 'utf8'))

    if (dropErr && existsSync(BASE_SQL) && !chunks.length) chunks.push(readFileSync(BASE_SQL, 'utf8'))

    if (mfpErr && existsSync(BASE_SQL) && !chunks.some(c => c.includes('source_program_dropoff_id'))) {

      chunks.push(readFileSync(BASE_SQL, 'utf8'))
